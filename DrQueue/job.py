...
class Job(dict):
    ...
    def __init__(self, name, startframe, endframe, blocksize, priority, renderer, scenefile, retries=1, owner=getpass.getuser(), options={}, created_with=None, limits={}, job_status=None):
        dict.__init__(self)
        # mandatory elements
        jb = {'name' : name,
    ...
    @staticmethod
    def query_job_status(job_status):
        """query job names from MongoDB"""
        jobs = get_jobs()
        jobs_with_status = list(jobs.find({"job_status": job_status}).sort("priority", pymongo.DESCENDING))
